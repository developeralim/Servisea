                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <strong class="card-title">LIST OF Job Requested by Project Owners</strong>
                                </div>
                                <div class="card-body">
                                    <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                        @if(isset($jr_List))
                                            @foreach($jr_List as $job)
                                            <tr>
                                                <td>{{$job->JR_ID}}</td>
                                                <td><a href="" class="button">{{$job->JR_TITLE}}</a></td>
                                                <td><a href="" class="button">{{$job->USERNAME}}</a></td>
                                                <td>
                                                    @if($job->JR_STATUS == "CONFIRMED")
                                                        <button type="button" class="btn btn-success" id="edit_btn">{{$job->JR_STATUS}}</button>
                                                    @elseif($job->JR_STATUS == "SUSPENDED")
                                                        <button type="button" class="btn btn-danger" id="edit_btn">{{$job->JR_STATUS}}</button>
                                                    @else
                                                        <button type="button" class="btn btn-warning" id="edit_btn">{{$job->JR_STATUS}}</button>
                                                    @endif
                                                </td>
                                                <td>
                                                @if($job->JR_STATUS == "SUSPENDED")
                                                   <a type="button" class="btn btn-success" href="{{route('admin_update_jr',Crypt::encryptString($job->JR_ID))}}">Re-Activate</a>
                                                @else
                                                   <a type="button" class="btn btn-danger" href="{{route('admin_update_jr',Crypt::encryptString($job->JR_ID))}}">Remove</a>
                                                @endif
                                                </td>
                                            </tr>
                                            @endforeach
